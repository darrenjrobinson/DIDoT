...
namespace AspNetCoreVerifiableCredentials
{
    ...
    public class VerifierController : Controller
    {
        ...
        public VerifierController(IOptions<AppSettingsModel> appSettings, IMemoryCache memoryCache, IMemoryCache credentialMemCache, ILogger<VerifierController> log, IHttpClientFactory httpClientFactory)
        {
            this.AppSettings = appSettings.Value;
            _cache = memoryCache;
            _credentialCache = credentialMemCache;
            _log = log;
            ...
            try
            {
                ...
                if (presentationResponse["code"].ToString() == "presentation_verified")
                {
                    var cacheData = new
                    {
                        ...
                        productName = presentationResponse["issuers"][0]["claims"]["productName"].ToString(),
                        allergenName = presentationResponse["issuers"][0]["claims"]["allergenName"].ToString(),
                        ingredientName = presentationResponse["issuers"][0]["claims"]["ingredientName"].ToString(),
                        presentationResponse = presentationResponse // need to cache the entire presentation response for B2C
                    };
                    _cache.Set(state, JsonConvert.SerializeObject(cacheData));
//// code to pass the data to ingredients.chtml.cs via _cache object 
//                if (_cache.TryGetValue("credentialData", out string dummyName)) { 

string credentialData = JsonConvert.SerializeObject(new {
    productName = presentationResponse["issuers"][0]["claims"]["productName"],
    allergenName = presentationResponse["issuers"][0]["claims"]["allergenName"],
    ingredientName = presentationResponse["issuers"][0]["claims"]["ingredientName"]
});




_credentialCache.Set("credentialData", credentialData);
//                }
//// end passing fo the data to ingredients
                }
